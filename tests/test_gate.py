...
class GateTests(unittest.TestCase):
    ...
    def test_fields_written_for_another_target_do_not_license_this_one(self) -> None:
        """The false-pass this check exists for.

        The gate reads the most recent assistant text. After one gated operation
        succeeds, those fields are still the most recent text, so the next
        destructive call would inherit them. Observed 2026-08-05.
        """
        out = self.run_gate(
            "rm -rf /tmp/some-other-directory",
            FIELDS_OK,  # written about a plugin install, not about this path
            calls=[("Bash", "ls /tmp")],
        )
        reason = self.assertAsks(out, "BINDING")
        self.assertIn("/tmp/some-other-directory", reason)
