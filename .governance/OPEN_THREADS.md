...
最后更新：2026-09-11T08:35Z · HEAD c04b797（推送后另有一个账本提交）

| 线程 | 问题 | 提出于 | 状态 |
|---|---|---|---|
| T-0007 | 工作分支名 `feat/v0.4.0` 与其内容（0.9.1、跟踪 `origin/main`、事实上的主干）不符，是否改名 | 2026-09-11 | 开放，等人裁定 |
| T-0009 | `.governance/` 随仓库被打进插件包，导致每次账本提交都让 `cache matches source` 报 FAIL、都要重装一次。是否把它排除出打包范围 | 2026-09-11 | 开放，等人裁定。**优先级高于另两条**：它会把一个真检查训练成背景噪音，正是 E-021 记录的失效形态 |
...
C-20260911-01 至 C-20260911-05 均为 `pending`。人尚未对其内容逐条盖章，
不得升级至 `decisions/`。
