...
| E-027 | *(resolved in v0.8 — fields moved onto the command; the entry stays as the reason)* The gate's field check depends on a race between transcript flush and hook execution | **Reproduced** (2026-08-05) | two calls of identical shape — fields written in the same assistant turn as the destructive command — one passed and one was denied for "FIELDS missing", with the text entry present and correctly ordered in the transcript both times | the visible direction is a false denial, which is safe. The invisible direction is worse: reading a stale turn surfaces an older turn's fields, which is E-025's false pass. BINDING was the only thing standing between that race and an unauthorised operation. Reading the fields from the transcript at all was the root cause. **Confirmed fixed in a live session, 2026-08-05:** under v0.6.1 the same install needed three attempts to get its fields seen; under v0.8.0 the gate denied a bare command, then accepted the identical command carrying the fields as comments, first try |
...
| E-028 | Governance traces left in the agent's own session storage sit on a retention timer nobody declared | **Observed** (2026-08-06) | Case 15 — about seventy transcript files on a working machine, oldest 33 days, nothing from the three preceding months of continuous use; a sibling directory under the same root still held files 100 days old, so the sweep was selective rather than a disk event; the mechanism was then confirmed from vendor documentation, after a first attempt matched the agent's own words in the current transcript | one machine, one vendor default, read once. What generalizes is that a store outside the project is not the project's to keep. The specific window does not generalize and must not be quoted as a constant |
| E-029 | Separating an agent's draft from a human's confirmation makes the agent's misdetection harmless | **Predictive** (2026-08-06) | reasoning, not observation: a false positive costs one file in a directory nothing downstream is allowed to trust, and a false negative is recoverable at the next grounding pass; neither outcome can promote itself. The mechanism ships in v0.9 and has produced no records yet | the separation is **auditable, not enforceable** — a hook cannot distinguish "the human agreed" from "the agent decided they did". Accordingly it never blocks: SessionEnd reports, and the injected line is advisory |
| E-030 | A PreToolUse check that assumes the gated call is already in the transcript discards the most recent evidence | **Observed** (2026-08-06) | Case 16 — three consecutive denials of a correctly evidenced plugin install, each preceded by a read-only inspection. `has_prior_evidence` sliced off the final entry per a comment asserting it was the gated call; the hook fires before that call is written, so the entry removed was the inspection itself. The gate's own function returned `True` on the same transcript when run afterwards | a false denial only, so the direction is safe. What it costs is standing: the gate reported "no read-only call precedes this one" while one did, three times in a row |
| E-031 | An anchored read-only pattern, applied to a whole invocation, does not recognise the ordinary shape of an inspection | **Observed** (2026-08-06) | Case 16 — `READ_ONLY_BASH` answers "does this command *begin* with a read-only verb", so `cd repo && git status` never matched. Nearly every real inspection in the session carried a `cd` prefix and was therefore invisible to the evidence check | fixed by classifying per operative segment and requiring **all** segments to pass, so `cd repo && ls && <mutation>` still does not count. Segment splitting must also ignore separators inside quotes, or a single quoted argument reads as many operations |
| E-032 | A test fixture that supplies more than the runtime does cannot fail on the defect it exists to catch | **Observed** (2026-08-06) | Case 16 — the gate suite built its transcript by appending the gated call, so the slice under test was always correct in tests and never correct in production. The defect shipped through a suite specifically written to exercise that code path | same family as E-020 (a suite green in CI, red on a real machine), but the mechanism is different: not environment leakage, fixture optimism. The regression test now deliberately omits the gated call |
| E-033 | A self-check that resolves its comparison baseline from its own location cannot fail | **Observed** (2026-09-11) | Case 17 — doctor's `cache matches source` hashed `install_path` against `PLUGIN_DIR`, which defaults to the script's own parent. Run from the installed cache, which is how the SessionStart line tells the operator to run it, those are one tree. Three files were out of sync and it reported PASS; the same bytes reported `FAIL 3 file(s) differ` when the baseline was pointed at the repository | one script, one check. What generalizes is the reporting rule — **a comparison must name both sides in its output**, because a pass that does not say what it compared against cannot be distinguished from a pass that compared nothing. What does not generalize is that every self-check has this shape |
| E-034 | A filter that recognises verbs cannot see an effect that lives inside a script filename | **Observed** (2026-09-11) | Case 18 — `ssh rig 'bash run_quant.sh'` and `ssh rig '<venv>/python graft.py'` started irreversible jobs on another machine with no gate firing, while `ssh rig 'rm -rf x'` was caught. The second only because its verb survived inside the quotes: the filter substring-matches the whole invocation, so quoting hides nothing and naming everything hides everything | the gap is not remote-specific — `bash run.sh` is equally invisible locally. What makes remote the place to fix first is that `VERIFY-AFTER` is hardest exactly where the effect landed somewhere this session cannot look. Measured on this operator's own 6157 remote commands, deciding from the payload rather than the verb newly gates 28% of them; that number is a property of this operator's workload, not a constant |
...
| E-027 |（v0.8 已解决——字段移到命令上;本条保留作为改动理由）门的字段检查依赖"transcript 落盘"与"hook 执行"之间的竞态 | **Reproduced**(2026-08-05) | 两次形状完全相同的调用——字段与破坏性命令写在同一个 assistant 轮次——**一次通过,一次以"FIELDS 缺失"被拒**,而两次的文本条目都在 transcript 里且顺序正确 | 可见的方向是误拒,安全。**不可见的方向更糟**:读到陈旧轮次就会看到更早那一轮的字段,即 E-025 的假放行。**BINDING 曾是唯一挡在这场竞态和一次未授权操作之间的东西**。根因是"从 transcript 里读字段"这件事本身。**2026-08-05 已在真实会话中确认修复:** 在 v0.6.1 下,同一次安装为了让字段被看到试了三次;在 v0.8.0 下,门先拒绝了裸命令,随后接受了同一条带注释字段的命令,**一次通过** |
...
| E-028 | 留在 agent 自己 session 存储里的治理痕迹,坐在一条没人声明过的保留期计时器上 | **Observed**(2026-08-06) | 案例 15——一台在用机器上约七十个 transcript 文件,最老 33 天,而此前**三个月的连续使用一条不剩**;同根目录下的兄弟目录仍留着 100 天前的文件,故清扫是**选择性的**、不是磁盘事件;机制随后由厂商文档确认——第一次搜索命中的是 agent 自己写在当前 transcript 里的话 | 单机、单一厂商默认值、单次读取。**可泛化的是**"项目之外的存储不归项目保管";**不可泛化的是**那个具体窗口,不得当作常量引用 |
| E-029 | 把 agent 的起草与人的确认分开,可使 agent 的误判变得无害 | **Predictive**(2026-08-06) | 推理而非观察:误报的代价是在一个下游本就不许采信的目录里多一个文件,漏报可在下一次 grounding 补回;**两种结果都无法自我升级**。机制随 v0.9 发布,尚未产生任何记录 | 这种分离是**可审计的,不是可强制的**——hook 分不清"人同意了"和"agent 认为人同意了"。因此它**永不拦截**:SessionEnd 只报告,注入行只是建议 |
| E-030 | 假设"被拦调用已在 transcript 里"的 PreToolUse 检查,会丢掉最近的那条证据 | **Observed**(2026-08-06) | 案例 16——一次取证正确的插件安装被连拒三次,每次紧邻的上一条都是只读检查。`has_prior_evidence` 依据一句"最后一条就是被拦调用"的注释把它切掉;而 hook 在该调用写入之前触发,被切掉的正是那次检查本身。事后把门自己的函数拿到同一份 transcript 上跑,返回 `True` | 只产生误拒,方向安全。**代价是信用**:门连续三次报告"这条前面没有只读调用",而每次都有 |
| E-031 | 行首锚定的只读模式套到整条调用上,认不出检查最常见的形状 | **Observed**(2026-08-06) | 案例 16——`READ_ONLY_BASH` 回答的是"这条命令是不是**以**只读动词开头",于是 `cd repo && git status` 永不匹配。该 session 里几乎每一次真实检查都带 `cd` 前缀,因此对取证检查全部不可见 | 修法是按**操作段**判断并要求**每一段**都通过,这样 `cd repo && ls && <变更>` 仍然不算取证。分段还必须忽略引号内的分隔符,否则一个带引号的参数会被读成许多个操作 |
| E-032 | 一个比运行时给得更多的测试夹具,不可能在它存在的目的上失败 | **Observed**(2026-08-06) | 案例 16——门的测试套件先把被拦调用追加进 transcript 再跑,于是被测的那个切片**在测试里永远正确、在生产中永远错误**。缺陷穿过了一套专为覆盖该代码路径而写的测试 | 与 E-020(CI 全绿、真机报错)同族,但机制不同:不是环境泄漏,是**夹具乐观**。新的回归测试**故意不把被拦调用放进 transcript** |
| E-033 | 自检脚本若从自身所在位置解析比对基准,则该检查不可能失败 | **Observed**(2026-09-11) | 案例 17——doctor 的 `cache matches source` 拿 `install_path` 与 `PLUGIN_DIR` 逐文件哈希,而后者缺省解析为脚本自己所在目录的上一级。从已安装的缓存里调用——正是 SessionStart 那行提示的调用方式——两者是同一棵树。当时有三个文件不同步,它报 PASS;把基准指向仓库后,同一批字节报 `FAIL 3 file(s) differ` | 单脚本、单检查。**可泛化的是报告纪律**:比对必须在输出里点名两侧,因为一个不说明「跟谁比」的通过,和一个根本没比的通过,从外面看没有区别。**不可泛化的是**认为所有自检都长这个形状 |
| E-034 | 认动词的过滤器，看不见藏在脚本文件名里的效果 | **Observed**(2026-09-11) | 案例 18——`ssh rig 'bash run_quant.sh'` 与 `ssh rig '<venv>/python graft.py'` 在另一台机器上启动了不可逆任务，门一次都没开火；而 `ssh rig 'rm -rf x'` 被抓住了。后者被抓只是因为它的动词在引号里活了下来：过滤器对整条调用做子串匹配，所以引号藏不住任何东西，而不写动词则藏住了一切 | 这个缺口并非远程独有——`bash run.sh` 在本地同样隐形。远程之所以该先修，是因为效果落在本 Session 看不到的地方，`VERIFY-AFTER` 恰恰在那里最难履行。在本操作者自己的 6157 条远程命令上实测：改为按载荷判定后新增开火 28%，该数字是此操作者工作负载的属性，不是常数 |
